...
@section("content")
    <div class="px-9 py-28 md:px-20">
        ...
        >
            <div class="flex h-full w-full flex-col md:w-1/2">
                ...
                <div class="text-justify text-sm md:text-base">
                    @php
                        ...
                        $content = preg_replace("/<h1>/", '<h1 class="text-md font-regular text-justify mb-5">', $content);

                        // Tambahkan kelas jika tag <h2> ditemukan
                        $content = preg_replace("/<h2>/", '<h2 class="text-md font-regular text-justify mb-5">', $content);

                        // Tambahkan kelas jika tag <h3> ditemukan
                        $content = preg_replace("/<h3>/", '<h3 class="text-md font-regular text-justify mb-5">', $content);

                        // Tambahkan kelas jika tag <p> ditemukan
                        $content = preg_replace("/<p>/", '<p class="text-md font-regular text-justify mb-5">', $content);

                        // Tambahkan kelas jika tag <li> ditemukan dalam <ol>
                        $content = preg_replace_callback(
                            "/<ol>(.*?)<\/ol>/s",
                            function ($matches) {
                                return '<ol class="text-md font-regular text-justify mb-5">' .
                                    preg_replace("/<li>/", '<li class="list-decimal text-md font-regular text-justify ml-5 mb-2">', $matches[1]) .
                                    "</ol>";
                            },
                            $content,
                        );
                        ...
                        $content = preg_replace_callback(
                            "/<ul>(.*?)<\/ul>/s",
                            function ($matches) {
                                return '<ul class="text-md font-regular text-justify mb-5">' .
                                    preg_replace("/<li>/", '<li class="list-disc text-md font-regular text-justify ml-5 mb-2">', $matches[1]) .
                                    "</ul>";
                            },
                            $content,
                        );
            ...
            <div class="flex h-full w-full flex-col md:w-1/2">
                <img
                    class="h-[480px] w-[700px] border border-black bg-cover"
