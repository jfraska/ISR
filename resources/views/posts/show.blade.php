...
@section('content')
    <article class="col-span-4 mx-auto mt-10 w-full py-5 md:col-span-3" style="max-width: 700px">
        <img class="my-2 w-full rounded-lg" src="{{ $post->getFirstMediaUrl() }}" alt="thumbnail" />
        <h1 class="text-left text-4xl font-bold text-gray-800">
            {{ $post->title }}
        </h1>
        <div class="mt-2 flex items-center justify-between">
            <div class="flex items-center py-5">
                <img class="mr-4 h-16 w-16 rounded-full" src="{{ $post->user->profile_photo_url }}"
                    alt="{{ $post->user->name }}" />
        ...
        @foreach ($post->content as $item)
            @if ($item['type'] === 'paragraph')
                <div class="article-content prose py-3 text-justify text-lg text-gray-800">
                    {!! $item['data']['content'] !!}
                </div>
            @endif
        @endforeach
